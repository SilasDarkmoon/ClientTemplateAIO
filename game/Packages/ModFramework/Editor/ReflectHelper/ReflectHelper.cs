...
namespace UnityEditorEx
{
    public static class ReflectHelper
    {
            ...
            if (asm != null)
            {
                try
                {
                    ...
                    if (file.StartsWith(assetdir, StringComparison.InvariantCultureIgnoreCase)
                        ...
                        || !string.IsNullOrEmpty(ModEditor.GetPackageNameFromPath(file)))
                    {
                        return true;
                    }
                }
        ...
        public static ScriptCategory GetScriptCategory(this Assembly asm)
        {
            ...
            if (asm != null)
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(package))
                    {
                        var part = file.Substring(ModEditor.GetPackageRoot(package).Length);
        ...
        private static string GetScriptMod(Type type, AssemblyDefinition ad, Mono.Cecil.Cil.ISymbolReader dr)
        {
            ...
            {
                var asset = ModEditorUtils.GetAssetNameFromPath(src);
                if (!string.IsNullOrEmpty(asset))
                {
                    return ModEditor.GetAssetModName(asset);
                }
            }
            return "";
        }
